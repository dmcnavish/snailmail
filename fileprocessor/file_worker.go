...
func createZipFile(sourceFileName, targetFileName string) error {
	if sourceFileName == targetFileName {
		return errors.New("sourceFileName cannot be the same as targetFileName")
	}
...
func UnzipAndJoin(sourceBaseName string) error {
    log.Println("sourceBaseName: ", sourceBaseName)
	zipFiles := []string{}
	...
	if len(zipFiles) == 0 {
		log.Fatal("No zip files found! Exiting")
	}

    destination := "_" + sourceBaseName
	for _, zipFile := range zipFiles {
		err := unzip(zipFile, destination)
		if err != nil {
			log.Printf("error unzipping file: %v \n %v",zipFile, err)
            return err
		}
	}

	err = joinFiles(destination, sourceBaseName)
	if err != nil {
		panic(err)
	}
	return nil
...
func unzip(source, destination string) error {
	...
	if err != nil {
        log.Printf("Error opening zip file %v", source)
		return err
	}
	defer func() {
	...
	for _, file := range reader.File {
		...
		if err != nil {
            log.Printf("error extracting file: %v", file.Name)
			return err
		}
	}
...
func extractAndWriteFile(file *zip.File, destination string) error {
    log.Println("extractAndWriteFile:", file.Name)
	rc, err := file.Open()
	if err != nil {
	...
	defer func() {
		if err := rc.Close(); err != nil {
            log.Printf("Panic!! error closing file: %v \n %v", file, err)
			panic(err)
		}
	}()
...
func joinFiles(destination, sourceBaseName string) error {
	files := []string{}
	...
	for _, fileName := range files {
		...
		for {
			...
			writeFile(destination+"\\"+sourceBaseName, buf[:n])
		}
	}
